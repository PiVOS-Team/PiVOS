...
static inline uint64_t bitmap_convert_to_mask(uint32_t number, uint32_t offset) {
    return (~0ULL << (PAGES_PER_ENTRY - number)) >> offset;  // NOLINT
}

static inline uint32_t bitmap_get_page_number(uint64_t addr) {
    return (addr & ~KERNEL_SPACE_MASK) / s_page_size;
}

static inline uint64_t bitmap_get_addres(uint32_t page_number) {
    return page_number * s_page_size;
}

static inline void bitmap_claim_specific_page(uint32_t page_number) {
    uint64_t mask = bitmap_convert_to_mask(1, page_number % PAGES_PER_ENTRY);
    s_bitmap[page_number / PAGES_PER_ENTRY] |= mask;
}

static inline void bitmap_free_specific_page(uint32_t page_number) {
    uint64_t mask = ~bitmap_convert_to_mask(1, page_number % PAGES_PER_ENTRY);
    s_bitmap[page_number / PAGES_PER_ENTRY] &= mask;
}

static void bitmap_claim_multiple_pages(uint32_t page_number, uint32_t count) {
    ...
    while (count > 0) {
        uint64_t mask = bitmap_convert_to_mask(pages_to_claim, offset);
        s_bitmap[entry] |= mask;
        entry++;
...
static void bitmap_free_multiple_pages(uint32_t page_number, uint32_t count) {
    uint32_t entry = page_number / PAGES_PER_ENTRY;
    uint32_t offset = page_number % PAGES_PER_ENTRY;
    uint32_t pages_to_free = MIN((PAGES_PER_ENTRY - offset), count);

    while (count > 0) {
        uint64_t mask = ~bitmap_convert_to_mask(pages_to_free, offset);
        s_bitmap[entry] &= mask;
        entry++;
        count -= pages_to_free;
...
void* alloc_bitmap_claim_page() {
    ...
    for (uint64_t entry_idx = 0; entry_idx < s_bitmap_entry_count; entry_idx++) {
        ...
        bitmap_claim_specific_page(page_number);
...
void* alloc_bitmap_claim_pages(uint32_t number, uint32_t align) {
    if (number == 1 && align == 1) {
        return alloc_bitmap_claim_page();
    }
    ...
    while (pages_left > 0 && entry_idx < s_bitmap_entry_count) {
        ...
        current_entry = s_bitmap[entry_idx];
        current_mask = bitmap_convert_to_mask(pages_to_claim, offset);
    ...
    if (pages_left == 0) {
        bitmap_claim_multiple_pages(current_page, number);
        return (void*)bitmap_get_addres(current_page);
    }
...
void alloc_bitmap_claim_range(void* from, void* to) {
    ...
    uint32_t to_page = bitmap_get_page_number((uint64_t)to);
...
void alloc_bitmap_free_range(void* from, void* to) {
    ...
    bitmap_free_multiple_pages(from_page, pages_to_free);
}